...
======================
  ...
  **Atención: Para poder usar el servidor, debes tener instalado [nodejs](http://nodejs.org).**
  ...
  ##### En consola:
      ...
      git clone https://github.com/tiko2015/naranju2.git
      cd naranju2
      npm install
      grunt build
      grunt server watch
  ...
  En el caso de stylus, se repite la misma mecánica. O sea que el llamado a style.css generaría que se compile stylus/style.css.styl con todos sus include y 
  devolviera una hoja de estilos única.

  > **NOTA:** Se pueden generar sitios estaticos si no se incluyen las carpetas stylus y views en el servidor de producción.
  
  ## Instruccion para trabajar con stylus en snippets
  ...
  - **stylus** (aca se encuentran los archivos .styl, no es necesaria para producción)
      - **lib**
          ...
          - **02extends** (librerías stylus para facilitar la creación de código)
  ...
  Lo primero que podés hacer es abrir el archivo *'stylus/style.styl'*. Acá vás a encotrar todos los archivos incluidos en tu proyecto y el orden en que se cargan.
  ...
  Para quitar archivos no es necesario borrarlos, se pueden comentar como el caso de *'reset.styl'*:

      ...
      //@import "01core/reset.styl";
      @import "01core/normalize.styl";
      ...
      
  Otro archivo donde nos proponemos trabajar bastante de nuestro tiempo es *04layout/all.styl*.
  ...
  Una de las grandes ventajas de crear css con stylus, es la posibilidad de usar *mixins*. Esto nos permite definir una serie de atributos para un elemento, 
  incluso se le pueden pasar variables.

  Vamos a ver un ejemplo de nuestro archivo *'01core/mixins.styl'*
  ...
  *Investiga que otros elementos tenes en mixins y agrega los que creas necesarios*.
  ...
  Con css3.styl haces compatible las propiedades de la nueva versión sin necesidad de escribir prefijos para todos los navegadores. 
  Mirá como usarlo en [stylus Prefixer](http://stylusprefixer.com/) 
